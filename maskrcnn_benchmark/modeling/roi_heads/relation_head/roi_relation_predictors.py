...
class TransformerPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.feats_compress = nn.Linear(self.pooling_dim + self.hidden_dim * 2, self.LAST_FEATS_DIM)
        self.feats_cls = nn.Linear(self.LAST_FEATS_DIM, self.num_rel_cls)
        layer_init(self.feats_compress, xavier=True)
        layer_init(self.feats_cls, xavier=True)
        self.lrelu = nn.LeakyReLU(0.1)

        self.feature_generation = config.MODEL.feats_generation_mode
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        last_feats = self.feats_compress(cat((visual_rep, prod_rep), dim=-1))
        last_feats = self.lrelu(last_feats)

        if self.feature_generation:
            rel_labels_all = cat(rel_labels, dim=0).long()
            fg_union_features = union_features[rel_labels_all.nonzero().reshape(-1), :]
            return last_feats.split(num_rels, dim=0), fg_union_features, pair_pred.split(num_rels, dim=0), rel_labels_all.split(num_rels, dim=0)

        rel_dists = self.feats_cls(last_feats)
