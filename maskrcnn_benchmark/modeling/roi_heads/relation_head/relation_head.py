...
class ROIRelationHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        """
        if self.training or cfg.MODEL.feats_generation_mode:
        ...
        if cfg.MODEL.feats_generation_mode:
            last_feats, fg_union_features, pair_pred, rel_labels = \
                self.predictor(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger)
            return {},[last_feats, fg_union_features, pair_pred, rel_labels],{}
        else:
