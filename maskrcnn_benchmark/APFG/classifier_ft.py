...
class Classifier(nn.Module):
    def __init__(self, config):
        ...
        self.cfg = config
        self.feats_cls = nn.Linear(self.LAST_FEATS_DIM, self.num_rel_cls)
        layer_init(self.feats_cls, xavier=True)

    def forward(self, feats):
        return self.feats_cls(feats)
...
def sgg_model_load_parameterss(model, classifier, distributed):
    ...
    with torch.no_grad():
            model.roi_heads.relation.predictor.feats_cls.weight.copy_(classifier.feats_cls.weight, non_blocking=True)
            model.roi_heads.relation.predictor.feats_cls.bias.copy_(classifier.feats_cls.bias, non_blocking=True)
    return model
...
def classifier_ft(cfg, local_rank, distributed, logger):
    ...
    num_batch = cfg.SOLVER.Classifier_batch // num_gpus
    cls_optimizer = make_optimizer(cfg, classifier, logger, rl_factor=float(num_batch))
    ...
    if checkpointer.has_checkpoint():
        ...
        load_mapping = {"feats_cls": "roi_heads.relation.predictor.feats_cls"}
    ...
    for iteration, (feats, rels) in enumerate(train_data_loader,start_iter):
    ...
    if not cfg.TEST.Skip_test:
        checkpointer.load(test_mode=True)
        sgg_model = sgg_model_load_parameterss(sgg_model, classifier, distributed)
