...
class Checkpointer(object):
    ...
    def save(self, name, **kwargs):
        ...
        if 'last_val_results' not in kwargs:
            kwargs['last_val_results'] = 0
        if 'best_val_results' not in kwargs:
            kwargs['best_val_results'] = kwargs['last_val_results']

        self.tag_last_checkpoint(save_file, kwargs['last_val_results'], kwargs['best_val_results'])
        # self.tag_last_checkpoint(save_file)


    def load(self, f=None, with_optim=True, update_schedule=False, load_mapping={}, test_mode=False):
        if self.has_checkpoint():
            # override argument with existing checkpoint
            f = self.get_checkpoint_file()

        if test_mode:
            best_model_path = os.path.join(self.save_dir, "best_checkpoint")
    ...
    def tag_last_checkpoint(self, last_filename, last_val_results, best_val_results):
    # def tag_last_checkpoint(self, last_filename):
        save_file = os.path.join(self.save_dir, "last_checkpoint")
        with open(save_file, "w") as f:
            f.write(last_filename)

        if last_val_results >= best_val_results:
